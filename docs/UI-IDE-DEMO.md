...
截图版统一要求：CLI/SDD 案例每例保留“主文件、码道案例卡、实际运行、独立验收”4 张图；Web 案例额外保留“专用服务启动、内置浏览器初始页面、交互结果”，每例共 6 张图。截图必须来自本例重新启动的终端和服务，不复用上一个案例状态。
...
- 账号当前返回套餐冻结/无功能访问权限；因此 `/demo-list` 已从 UI 发起，但 AI 返回结果未通过，未被计入成功项。
